...
local function safeStart()
	local success, err = pcall(function()
		...
		local function startMain()
			...
			local tabAutoBlock  = Window:AddTab('自动格挡', 'shield')  -- ★ 自动格挡专用
			print("✅ 所有选项卡创建完成")
			...
			local secInterface = tabSettings:AddLeftGroupbox("界面")
			...
			secInterface:AddToggle("showChatLogs", {
				Text = "显示聊天记录",
			...
			applyTimerPos()
			secInterface:AddDropdown("timerPos", {
				Text = "计时器位置",
				Values = { "中间", "右侧" },
				Default = timerSide,
			...
			local secPlatform = tabSettings:AddLeftGroupbox("平台伪装")
			...
			-- ==================== 视觉 (ESP) ====================
			local secESP = tabVisual:AddLeftGroupbox("ESP")
			local esp = {
			...
			secESP:AddToggle("espKillers", { Text="杀手", Default=esp.killers, Callback=function(on) esp.killers=on; cfg.set("espKillers",on); espDoKillers(on) end })
			secESP:AddToggle("espSurvivors", { Text="幸存者", Default=esp.survivors, Callback=function(on) esp.survivors=on; cfg.set("espSurvivors",on); espDoSurvivors(on) end })
			secESP:AddToggle("espGenerators", { Text="发电机", Default=esp.generators, Callback=function(on) esp.generators=on; cfg.set("espGenerators",on); espDoGenerators(on) end })
			secESP:AddToggle("espItems", { Text="物品", Default=esp.items, Callback=function(on) esp.items=on; cfg.set("espItems",on); espDoItems(on) end })
			secESP:AddToggle("espBuildings", { Text="建筑", Default=esp.buildings, Callback=function(on) esp.buildings=on; cfg.set("espBuildings",on); espDoBuildings(on) end })

			local secMinion = tabVisual:AddLeftGroupbox("小兵与技能 ESP")
			...
			local function scanPizza()
				...
				for _,obj in ipairs(svc.WS:GetDescendants()) do if obj.Name=="PizzaDeliveryRig" and obj:IsA("Model") and not isRealPlayer(obj) and not tracked.pizza[obj] then addHighlight(obj,Color3.fromRGB(255,100,0),"pizza","C00LKIDD 披萨配送",3) end end
			end
			local function scanZombie()
				if not mset.zombie then return end
				for _,obj in ipairs(svc.WS:GetDescendants()) do if obj.Name=="1x1x1x1Zombie" and obj:IsA("Model") and not isRealPlayer(obj) and not tracked.zombie[obj] then addHighlight(obj,Color3.fromRGB(80,255,120),"zombie","1X1X1X1 僵尸",3) end end
			end
			local function scanPuddles()
				if not mset.puddle then return end
				for _,obj in ipairs(svc.WS:GetDescendants()) do if isJohnDoePuddle(obj) and not tracked.puddle[obj] then addPuddleHighlight(obj,Color3.fromRGB(255,50,50),"puddle","JOHN DOE 水洼") end end
			end
			local function setupMinionWatcher()
				svc.WS.DescendantAdded:Connect(function(obj)
					task.defer(function()
						if not obj or not obj.Parent then return end
						if mset.pizza  and obj.Name=="PizzaDeliveryRig"  and obj:IsA("Model") and not isRealPlayer(obj) and not tracked.pizza[obj]  then addHighlight(obj,Color3.fromRGB(255,100,0),"pizza","C00LKIDD 披萨配送",3) end
						if mset.zombie and obj.Name=="1x1x1x1Zombie"     and obj:IsA("Model") and not isRealPlayer(obj) and not tracked.zombie[obj] then addHighlight(obj,Color3.fromRGB(80,255,120),"zombie","1X1X1X1 僵尸",3) end
						if mset.puddle and isJohnDoePuddle(obj) and not tracked.puddle[obj] then addPuddleHighlight(obj,Color3.fromRGB(255,50,50),"puddle","JOHN DOE 水洼") end
					end)
				end)
			end
			...
			secMinion:AddToggle("espPizza", { Text="c00lkidd 披萨机器人", Default=mset.pizza, Callback=function(on) mset.pizza=on; cfg.set("espPizza",on); if on then scanPizza() else clearTag("pizza") end end })
			secMinion:AddToggle("espZombie", { Text="1x1x1x1 僵尸", Default=mset.zombie, Callback=function(on) mset.zombie=on; cfg.set("espZombie",on); if on then scanZombie() else clearTag("zombie") end end })
			secMinion:AddToggle("espPuddle", { Text="JD 数字足迹", Default=mset.puddle, Callback=function(on) mset.puddle=on; cfg.set("espPuddle",on); if on then scanPuddles() else clearTag("puddle") end end })
			secMinion:AddSlider("espMinionTrans", { Text="高亮透明度", Default=mset.transparency, Min=0, Max=1, Rounding=2, Callback=function(v) mset.transparency=v; cfg.set("espMinionTrans",v); updateTransparency() end })
			secMinion:AddButton({ Text="🔄 强制重新扫描", Func=function() clearTag("pizza"); clearTag("zombie"); clearTag("puddle"); task.defer(function() scanPizza(); scanZombie(); scanPuddles() end) end })
    ...
    ["116468089135195"]=true,["112809109188560"]=true,["109348678063422"]=true,
			...
			local AttackSoundIDs = autoBlockTriggerSounds
...
-----				destroyConn:Disconnect()
			...
			local function watchKillerAnimations(killerModel)
				...
				local conn = animator.AnimationPlayed:Connect(function(track)
					...
					pressBlock()
					animBlockedUntil[killerModel] = t + 0.1
				end)
